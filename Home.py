...
with col1:
    st.write("")
    st.write("")
    st.image("images/Tehran_01.jpg")
    st.write("")
    st.write("")
    st.image("images/Tehran_02.jpg")
...
col3, spaceCol, col4 = st.columns([1, 0.25, 1])

with col3:

    for index, row in df.iloc[:33].iterrows():
        st.title(row['name'])
        st.image('images/' + row['image'])
        st.markdown(f"[Location]({row['location']})")
        st.markdown(f"[Other Photos]({row['other photos']})")
